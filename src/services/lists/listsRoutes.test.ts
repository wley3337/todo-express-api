...
describe("Lists Routes", () => {
  ...
  beforeAll(async () => {
    ...
    testUser = await db.one(
      "INSERT INTO users(first_name, last_name, username, password_digest, created_at, updated_at) VALUES($1, $2, $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING *",
      [
        "testFirstName",
        "testLastName",
        "test123",
        passwordDigestResponseObj.passwordDigest
      ]
    );
  });
  beforeEach(() => {
  ...
  describe("delete /lists/:id", () => {
    ...
    test("List is properly deleted and deletes all toDos associated with list. Returns old list", async () => {
      const testToken = generateJWT({ user_id: testUser.id });
      testList = await db.one(
        "INSERT INTO lists(heading, user_id, display_order, created_at, updated_at) VALUES($1, $2, $3, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING *",
        ["Test List", testUser.id, 0]
      );
      console.log("Test List ID: ", testList.id);

      const testToDo = await db.one(
        "INSERT INTO to_dos(list_id, title, description, due, created_at, updated_at) VALUES($1,$2,$3,$4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING *",
        [testList.id, "test toDo", "test toDo description", "2019-12-01"]
      );

      const serializedToDo: SerializedToDo = {
        id: testToDo.id,
        listId: testToDo.list_id,
        title: testToDo.title,
        description: testToDo.description,
        due: testToDo.due
      };

      const serializedList: SerializedListType = {
        id: parseInt(testList.id),
        heading: testList.heading,
      ...
      const response = await request(router)
        ...
        .set("authorization", `Bearer ${testToken}`)
        .send({ list: serializedList });
      const shouldBeEmpty = await db.any("SELECT * FROM to_dos WHERE id = $1", [
        serializedToDo.id
      ]);
